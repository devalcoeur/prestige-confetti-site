...
function setMusicUI(on){
  ...
  sound.setAttribute('aria-label',on?'Mute sound':'Turn on sound');
}
...
document.querySelector('form').addEventListener('submit',e=>{e.preventDefault();alert('This form is currently a demo. Enquiry submission will be enabled when the website goes live.');});
...
confettiToggle?.addEventListener('click',async()=>{
  ...
  confettiToggle.textContent=confettiOn?'Confetti: on':'Confetti: off';
  confettiToggle.setAttribute('aria-label',confettiOn?'Turn off confetti':'Turn on confetti');
