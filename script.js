...
document.querySelector('form').addEventListener('submit',e=>{e.preventDefault();alert('Форма пока демонстрационная. Подключим отправку заявок при публикации сайта.');});
...
confettiToggle?.addEventListener('click',async()=>{
  ...
  confettiToggle.textContent=confettiOn?'Конфетти: вкл.':'Конфетти: выкл.';
  confettiToggle.setAttribute('aria-label',confettiOn?'Выключить эффект конфетти':'Включить эффект конфетти');
